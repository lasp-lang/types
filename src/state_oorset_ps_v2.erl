...
delta_mutate({add, Elem},
    ...
    DeltaDataStore = orddict:store(Elem, [[NewEvent]], orddict:new()),
    {ok, {?TYPE, {delta, {DeltaDataStore, ordsets:new(), [NewEvent]}}}};
    ...
    {AccDeltaDataStore, AccDeltaAllEvents, _AccAllEvents} =
        lists:foldl(
          fun(Elem, {AccDeltaDataStore0, AccDeltaAllEvents0, AccAllEvents0}) ->
                  ...
                  {orddict:store(Elem, [[NewEvent]], AccDeltaDataStore0),
...
delta_mutate({rmv, Elem},
    ...
    case orddict:find(Elem, DataStore) of
        {ok, Provenance} ->
            ...
            {ok, {?TYPE, {delta, {orddict:new(), ordsets:new(), ElemEvents}}}};
        error ->
...
join_provenance(ProvenanceA, ProvenanceB, AllEventsA, AllEventsB,
    ...
    ordsets:fold(
      ...
      end, ordsets:new(), ordsets:union(ProvenanceA, ProvenanceB)).
