...
-export_type([state_oorset_ps_v2/0,
              delta_state_oorset_ps_v2/0,
              state_oorset_ps_v2_op/0]).

-opaque state_oorset_ps_v2() :: {?TYPE, payload()}.
-opaque delta_state_oorset_ps_v2() :: {?TYPE, {delta, payload()}}.
-type delta_or_state() :: state_oorset_ps_v2() | delta_state_oorset_ps_v2().
-type payload() :: {ps_data_store(), ps_survived_events(), ps_all_events()}.
-type element() :: term().
-type state_oorset_ps_v2_op() :: {add, element()}
                               | {add_all, [element()]}
                               | {rmv, element()}
                               | {rmv_all, [element()]}.
...
%% @doc Create a new, empty `state_oorset_ps_v2()'
-spec new() -> state_oorset_ps_v2().
new() ->
    {?TYPE, {orddict:new(), ordsets:new(), {vclock, []}}}.

%% @doc Create a new, empty `state_oorset_ps_v2()'
-spec new([term()]) -> state_oorset_ps_v2().
new([]) ->
    new().

%% @doc Mutate a `state_oorset_ps_v2()'.
-spec mutate(state_oorset_ps_v2_op(), type:id(), state_oorset_ps_v2()) ->
    {ok, state_oorset_ps_v2()} | {error, {precondition, {not_present, [element()]}}}.
mutate(Op, Actor, {?TYPE, _ORSet}=CRDT) ->
    state_type:mutate(Op, Actor, CRDT).

%% @doc Delta-mutate a `state_oorset_ps_v2()'.
%%      The first argument can be:
%%          - `{add, element()}'
%%          - `{add_all, [element()]}'
%%          - `{rmv, element()}'
%%      The second argument is the event id ({object_id, replica_id}).
%%      The third argument is the `state_oorset_ps_v2()' to be inflated.
-spec delta_mutate(state_oorset_ps_v2_op(), type:id(), state_oorset_ps_v2()) ->
    {ok, delta_state_oorset_ps_v2()} | {error, {precondition, {not_present, element()}}}.
%% Adds a single element to `state_oorset_ps_v2()'.
...
%% Adds a list of elemenets to `state_oorset_ps_v2()'.
...
%% Removes a single element in `state_oorset_ps_v2()'.
%% Delta: {[], (ElemEvents - OwnedElemEvents), ElemEvents}
%% OwnedElemEvents = the object id is equal to the one of the `state_oorset_ps_v2()'
...
%% @doc Equality for `state_oorset_ps_v2()'.
-spec equal(state_oorset_ps_v2(), state_oorset_ps_v2()) -> boolean().
equal({?TYPE, {DataStoreA, SurvivedEventsA, {vclock, AllEventsA}}},
      {?TYPE, {DataStoreB, SurvivedEventsB, {vclock, AllEventsB}}}) ->
    DataStoreA == DataStoreB andalso
        SurvivedEventsA == SurvivedEventsB andalso
        lists:sort(AllEventsA) == lists:sort(AllEventsB).

%% @doc Given two `state_oorset_ps_v2()', check if the second is and inflation of the first.
%% The inflation will be checked by the is_lattice_inflation() in the common library.
-spec is_inflation(state_oorset_ps_v2(), state_oorset_ps_v2()) -> boolean().
is_inflation({?TYPE, ORSet1}, {?TYPE, ORSet2}) ->
    is_lattice_inflation_oorset_ps(ORSet1, ORSet2).

%% @doc Check for strict inflation.
-spec is_strict_inflation(state_oorset_ps_v2(), state_oorset_ps_v2()) -> boolean().
is_strict_inflation({?TYPE, _}=CRDT1, {?TYPE, _}=CRDT2) ->
    state_type:is_strict_inflation(CRDT1, CRDT2).

%% @todo
%% @doc Join decomposition for `state_oorset_ps_v2()'.
-spec join_decomposition(state_oorset_ps_v2()) -> [state_oorset_ps_v2()].
...
join_oorset_ps({DataStoreA, SurvivedEventsA, AllEventsA}=FstORSet,
    ...
    JoinedSurvivedEvents =
        ordsets:union([ordsets:intersection(SurvivedEventsA, SurvivedEventsB)] ++
                      [subtract_survived_events(SurvivedEventsA, AllEventsB)] ++
                      [subtract_survived_events(SurvivedEventsB, AllEventsA)]),
    {JoinedDataStore, JoinedSurvivedEvents, join_all_events(AllEventsA, AllEventsB)}.
...
%% @private
-spec subtract_survived_events(ps_survived_events(), ps_all_events()) ->
          ps_survived_events().
subtract_survived_events(SurvivedEvents, {vclock, AllEvents}) ->
    ordsets:fold(
      fun({EventId0, Counter0}=Event0, ResultSurvivedEvents0) ->
              case Counter0 > get_counter(EventId0, {vclock, AllEvents}) of
                  true ->
                      ordsets:add_element(Event0, ResultSurvivedEvents0);
                  false ->
                      ResultSurvivedEvents0
              end
      end, ordsets:new(), SurvivedEvents);
subtract_survived_events(SurvivedEvents, AllEvents) ->
    ordsets:subtract(SurvivedEvents, AllEvents).
...
is_valid_dot(Dot, InterSurvived, {vclock, AllEvents}) ->
    ordsets:fold(fun({EventId0, Counter0}=Event0, IsValid0) ->
                         (ordsets:is_element(Event0, InterSurvived) orelse
                              %%(not ordsets:is_element(Event0, AllEvents)))
                              (Counter0 > get_counter(EventId0, {vclock, AllEvents})))
                         andalso IsValid0
                 end, true, Dot);
...
merge_commutative_test() ->
    ...
    Set1_2 = {?TYPE, {[{<<"2">>, [[{EventId2, 1}]]}],
                      [{EventId2, 1}],
                      {vclock, [{EventId1, 1}, {EventId2, 1}]}}},
    Set1_3 = {?TYPE, {[], [], {vclock, [{EventId1, 1}, {EventId2, 1}]}}},
    Set2_3 = Set3,
...
merge_delta_test() ->
    ...
    ?assertEqual({?TYPE, {[], [], {vclock, [{EventId, 1}]}}}, Set2),
    ?assertEqual({?TYPE, {[], [], {vclock, [{EventId, 1}]}}}, Set3),
