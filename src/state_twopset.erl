...
-spec new_delta() -> delta_state_twopset().
new_delta() ->
    state_type:new_delta(?TYPE).

-spec new_delta([term()]) -> delta_state_twopset().
new_delta([]) ->
    new_delta().
...
new_test() ->
    ...
    ?assertEqual({?TYPE, {delta, {ordsets:new(), ordsets:new()}}}, new_delta()).
...
merge_deltas_test() ->
    ...
    ?assertEqual({?TYPE, {delta, {[<<"c">>], [<<"a">>, <<"b">>]}}}, DeltaGroup).
