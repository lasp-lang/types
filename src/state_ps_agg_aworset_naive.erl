...
-module(state_ps_agg_aworset_naive).
...
-export_type([
    state_ps_agg_aworset_naive/0,
    state_ps_agg_aworset_naive_op/0]).

-type element() :: term().
-type payload() :: state_ps_agg_poe_orset:state_ps_agg_poe_orset().
-opaque state_ps_agg_aworset_naive() :: {?TYPE, payload()}.
-type state_ps_agg_aworset_naive_op() :: {add, element()}
                                       | {rmv, element()}.

%% @doc Create a new, empty `state_ps_agg_aworset_naive()'.
-spec new() -> state_ps_agg_aworset_naive().
new() ->
    {?TYPE, state_ps_agg_poe_orset:new()}.

%% @doc Create a new, empty `state_ps_agg_aworset_naive()'
-spec new([term()]) -> state_ps_agg_aworset_naive().
new([_]) ->
    new().

%% @doc Mutate a `state_ps_agg_aworset_naive()'.
-spec mutate(
    state_ps_agg_aworset_naive_op(),
    type:id(),
    state_ps_agg_aworset_naive()) -> {ok, state_ps_agg_aworset_naive()}.
mutate(Op, Actor, {?TYPE, _}=CRDT) ->
    state_ps_agg_type:mutate(Op, Actor, CRDT).

%% @doc Returns the value of the `state_ps_agg_aworset_naive()'.
%%      This value is a set of not-removed elements.
-spec query(state_ps_agg_aworset_naive()) -> term().
query({?TYPE, Payload}) ->
    state_ps_agg_poe_orset:read(Payload).

%% @doc Equality for `state_ps_agg_aworset_naive()'.
-spec equal(state_ps_agg_aworset_naive(), state_ps_agg_aworset_naive()) ->
    boolean().
equal({?TYPE, PayloadA}, {?TYPE, PayloadB}) ->
    state_ps_agg_poe_orset:equal(PayloadA, PayloadB).

%% @doc Delta-mutate a `state_ps_agg_aworset_naive()'.
%%      The first argument can be:
%%          - `{add, element()}'
%%          - `{rmv, element()}'
%%      The second argument is the event id ({object_id, replica_id}).
%%      The third argument is the `state_ps_agg_aworset_naive()' to be inflated.
-spec delta_mutate(
    state_ps_agg_aworset_naive_op(),
    type:id(),
    state_ps_agg_aworset_naive()) -> {ok, state_ps_agg_aworset_naive()}.
%% Add a single element to `state_ps_agg_aworset_naive()'.
%% Delta: {[{Elem, [{undefined, {{NewEvent}}}]}], [NewEvent], [NewEvent]}
delta_mutate({add, Elem}, Actor, {?TYPE, Payload}) ->
    %% Get next Event from AllEvents.
    NextEvent = get_next_event(Actor, Payload),
    %% Get a delta object.
    DeltaPayload =
        state_ps_agg_poe_orset:delta_insert(NextEvent, Elem, Payload),
    {ok, {?TYPE, DeltaPayload}};

%% Remove a single element to `state_ps_agg_aworset_naive()'.
%% Delta: {[], [], ElemEvents}
delta_mutate({rmv, Elem}, _Actor, {?TYPE, Payload}) ->
    %% Get a delta object.
    DeltaPayload = state_ps_agg_poe_orset:delta_delete(Elem, Payload),
    {ok, {?TYPE, DeltaPayload}}.

%% @doc Merge two `state_ps_agg_aworset_naive()'.
-spec merge(state_ps_agg_aworset_naive(), state_ps_agg_aworset_naive()) ->
    state_ps_agg_aworset_naive().
merge({?TYPE, _}=CRDT1, {?TYPE, _}=CRDT2) ->
    MergeFun = fun merge_state_ps_agg_aworset_naive/2,
    state_ps_agg_type:merge(CRDT1, CRDT2, MergeFun).

%% @private
merge_state_ps_agg_aworset_naive({?TYPE, PayloadA}, {?TYPE, PayloadB}) ->
    MergedPayload = state_ps_agg_poe_orset:join(PayloadA, PayloadB),
    {?TYPE, MergedPayload}.

%% @doc Given two `state_ps_agg_aworset_naive()', check if the second is an
%%     inflation of the first.
-spec is_inflation(
    state_ps_agg_aworset_naive(), state_ps_agg_aworset_naive()) -> boolean().
...
-spec encode(state_ps_agg_type:format(), state_ps_agg_aworset_naive()) ->
    binary().
    ...
    MaxCnt =
        ordsets:fold(
            fun({EventType0, {EventId0, Counter0}}, AccInMaxCnt) ->
                case EventType0 == state_ps_agg_event_partial_order_independent
            ...
            0,
            AllEvents),
    {state_ps_agg_event_partial_order_independent, {EventId, MaxCnt + 1}}.
...
-define(EVENT_TYPE, state_ps_agg_event_partial_order_independent).
...
merge_idempotent_test() ->
    ...
    Set2 =
        {?TYPE, {
            [{<<"2">>, {dots, [[{?EVENT_TYPE, {EventId2, 1}}]]}}],
            [{?EVENT_TYPE, {EventId2, 1}}],
            [{?EVENT_TYPE, {EventId2, 1}}]}},
    Set3 =
        {?TYPE, {
            [{<<"1">>, {dots, [[{?EVENT_TYPE, {EventId1, 1}}]]}}],
            [{?EVENT_TYPE, {EventId1, 1}}],
