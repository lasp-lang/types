...
%% @doc Pure ORMap CRDT: pure op-based observed-remove Map
%%
...
-export_type([pure_ormap/0, pure_ormap_op/0]).

-opaque pure_ormap() :: {?TYPE, payload()}.
-type payload() :: {pure_type:polog(), ordsets:set()}.
-type pure_ormap_op() :: {add, pure_type:element()} | {rmv, pure_type:element()}.

%% @doc Create a new, empty `pure_ormap()'
-spec new() -> pure_ormap().
new() ->
    {?TYPE, {orddict:new(), ordsets:new()}}.

%% @doc Create a new, empty `pure_ormap()'
-spec new([term()]) -> pure_ormap().
new([]) ->
    new().

%% @doc Check redundancy `pure_ormap()'
%% Called in remove_redundant().
-spec redundant({pure_type:id(), pure_ormap_op()}, {pure_type:id(), pure_ormap_op()}) ->
    integer().
...
%% @doc Removes redundant operations from POLog of `pure_ormap()'
%% Called upon updating (add, rmv) the `pure_ormap()'
-spec remove_redundant_polog({pure_type:id(), pure_ormap_op()}, pure_ormap()) -> {boolean(), pure_ormap()}.
...
%% @doc Removes redundant operations from POLog of `pure_ormap()'
%% Called upon updating (add, rmv) the `pure_ormap()'
-spec remove_redundant_crystal({pure_type:id(), pure_ormap_op()}, pure_ormap()) -> {boolean(), pure_ormap()}.
remove_redundant_crystal({_VV1, {_X, Elem}}, {?TYPE, {POLog, ORMap}}) ->
    case ordsets:is_element(Elem, ORMap) of
        true ->
            {true, {?TYPE, {POLog, ordsets:del_element(Elem, ORMap)}}};
        false ->
            {true, {?TYPE, {POLog, ORMap}}}
    end.

%% @doc Checks stable operations and remove them from POLog of `pure_ormap()'
-spec check_stability(pure_type:id(), pure_ormap()) -> pure_ormap().
check_stability(StableVV, {?TYPE, {POLog0, ORMap0}}) ->
    {POLog1, ORMap1} = orddict:fold(
...
%% @doc Update a `pure_ormap()'.
-spec mutate(pure_ormap_op(), pure_type:id(), pure_ormap()) ->
    {ok, pure_ormap()}.
mutate({add, Elem}, VV, {?TYPE, {POLog, PureORMap}}) ->
    {_, {?TYPE, {POLog0, PureORMap0}}} = pure_polog:remove_redundant({VV, {add, Elem}}, {?TYPE, {POLog, PureORMap}}),
    {ok, {?TYPE, {orddict:store(VV, {add, Elem}, POLog0), PureORMap0}}};
mutate({rmv, Elem}, VV, {?TYPE, {POLog, PureORMap}}) ->
    {_, {?TYPE, {POLog0, PureORMap0}}} = pure_polog:remove_redundant({VV, {rmv, Elem}}, {?TYPE, {POLog, PureORMap}}),
    {ok, {?TYPE, {POLog0, PureORMap0}}}.

%% @doc Returns the value of the `pure_ormap()'.
%%      This value is a set with all the elements in the `pure_ormap()'.
-spec query(pure_ormap()) -> sets:set(pure_type:element()).
query({?TYPE, {POLog0, PureORMap0}}) ->
    Elements0 = ordsets:to_list(PureORMap0),
    Elements1 = [El || {_Key, {_Op, El}} <- orddict:to_list(POLog0)],
    sets:from_list(lists:append(Elements0, Elements1)).


%% @doc Equality for `pure_ormap()'.
-spec equal(pure_ormap(), pure_ormap()) -> boolean().
equal({?TYPE, {POLog1, PureORMap1}}, {?TYPE, {POLog2, PureORMap2}}) ->
    Fun = fun(Value1, Value2) -> Value1 == Value2 end,
    ordsets_ext:equal(PureORMap1, PureORMap2) andalso orddict_ext:equal(POLog1, POLog2, Fun).
...
remove_redundant_crystal_test() ->
    {Redundant0, {?TYPE, {_POLog0, ORMap0}}} = remove_redundant_crystal({[{0, 1}, {1, 2}, {2, 3}], {add, <<"a">>}}, {?TYPE, {[{0, 1}], [<<"a">>, <<"b">>, <<"c">>]}}),
    ?assertEqual(true, Redundant0),
    ?assertEqual([<<"b">>, <<"c">>], ORMap0),
    {Redundant1, {?TYPE, {_POLog1, ORMap1}}} = remove_redundant_crystal({[{0, 1}, {1, 2}, {2, 3}], {rmv, <<"a">>}}, {?TYPE, {[{0, 1}], [<<"a">>, <<"b">>, <<"c">>]}}),
    ?assertEqual(true, Redundant1),
    ?assertEqual([<<"b">>, <<"c">>], ORMap1),
    {Redundant2, {?TYPE, {_POLog2, ORMap2}}} = remove_redundant_crystal({[{0, 1}], {rmv, <<"d">>}}, {?TYPE, {[{0, 1}], [<<"a">>]}}),
    ?assertEqual(true, Redundant2),
    ?assertEqual([<<"a">>], ORMap2).
