...
-export_type([
    state_ps_size_t_naive/0,
    state_ps_size_t_naive_op/0]).

-type payload() :: state_ps_poe_orset:state_ps_poe_orset().
-opaque state_ps_size_t_naive() :: {?TYPE, payload()}.
-type state_ps_size_t_naive_op() :: no_op.

%% @doc Create a new, empty `state_ps_size_t_naive()'.
-spec new() -> state_ps_size_t_naive().
new() ->
    {?TYPE, state_ps_poe_orset:new()}.

%% @doc Create a new, empty `state_ps_size_t_naive()'
-spec new([term()]) -> state_ps_size_t_naive().
new([_]) ->
    new().

%% @doc Mutate a `state_ps_size_t_naive()'.
-spec mutate(
    state_ps_size_t_naive_op(), type:id(), state_ps_size_t_naive()) ->
    {ok, state_ps_size_t_naive()}.
mutate(_Op, _Actor, {?TYPE, _}=CRDT) ->
    {ok, CRDT}.

%% @doc Returns the value of the `state_ps_size_t_naive()'.
-spec query(state_ps_size_t_naive()) -> term().
query({?TYPE, Payload}) ->
    ...
    case InternalSet of
        [] ->
            0;
        [ValueAsProvenance] ->
            ordsets:size(ValueAsProvenance)
    end.

%% @doc Equality for `state_ps_size_t_naive()'.
%%      Since everything is ordered, == should work.
-spec equal(state_ps_size_t_naive(), state_ps_size_t_naive()) -> boolean().
equal({?TYPE, PayloadA}, {?TYPE, PayloadB}) ->
    state_ps_poe_orset:equal(PayloadA, PayloadB).

%% @doc Delta-mutate a `state_ps_size_t_naive()'.
%%      The first argument can only be `increment'.
%%      The second argument is the event id ({object_id, replica_id}).
%%      The third argument is the `state_ps_size_t_naive()' to be inflated.
-spec delta_mutate(
    state_ps_size_t_naive_op(),
    type:id(),
    state_ps_size_t_naive()) -> {ok, state_ps_size_t_naive()}.
%% Increase the value of the `state_ps_size_t_naive()'.
delta_mutate(no_op, _Actor, {?TYPE, Payload}) ->
    {ok, {?TYPE, Payload}}.

%% @doc Merge two `state_ps_size_t_naive()'.
-spec merge(state_ps_size_t_naive(), state_ps_size_t_naive()) ->
    state_ps_size_t_naive().
merge({?TYPE, _}=CRDT1, {?TYPE, _}=CRDT2) ->
    MergeFun = fun merge_state_ps_size_t_naive/2,
    state_ps_type:merge(CRDT1, CRDT2, MergeFun).

%% @doc Given two `state_ps_size_t_naive()', check if the second is an
%%      inflation of the first.
-spec is_inflation(state_ps_size_t_naive(), state_ps_size_t_naive()) ->
    boolean().
...
-spec is_strict_inflation(
    state_ps_size_t_naive(), state_ps_size_t_naive()) -> boolean().
is_strict_inflation({?TYPE, _}=CRDT1, {?TYPE, _}=CRDT2) ->
    state_ps_type:is_strict_inflation(CRDT1, CRDT2).

-spec encode(state_ps_type:format(), state_ps_size_t_naive()) -> binary().
encode(erlang, {?TYPE, _}=CRDT) ->
    erlang:term_to_binary(CRDT).

-spec decode(state_ps_type:format(), binary()) -> state_ps_size_t_naive().
...
-define(SET_EVENT_TYPE, state_ps_event_partial_order_independent).
-define(EVENT_TYPE, state_ps_event_partial_order_provenance).
        ...
        {?TYPE, {
            [{
                [
                    ...
                    [{?EVENT_TYPE, [[{?SET_EVENT_TYPE, {EventId1, 2}}]]}]],
                [
                    [{?EVENT_TYPE, [
                        [{?SET_EVENT_TYPE, {EventId1, 1}}],
                        [{?SET_EVENT_TYPE, {EventId2, 1}}]]}],
        ...
        {?TYPE, {
            [{
                [
                    ...
                    [{?EVENT_TYPE, [[{?SET_EVENT_TYPE, {EventId2, 2}}]]}]],
                [
                    [{?EVENT_TYPE, [
                        [{?SET_EVENT_TYPE, {EventId2, 1}}],
                        [{?SET_EVENT_TYPE, {EventId3, 1}}]]}],
