...
%% @doc Size CRDT for the AW OR Set with the POE OR Set design:
%%     size_t type.
...
-spec equal(
    state_ps_size_t_naive(), state_ps_size_t_naive()) ->
    boolean().
...
-spec encode(
    state_ps_type:format(), state_ps_size_t_naive()) -> binary().
encode(erlang, {?TYPE, _}=CRDT) ->
    erlang:term_to_binary(CRDT).

-spec decode(
    state_ps_type:format(), binary()) -> state_ps_size_t_naive().
